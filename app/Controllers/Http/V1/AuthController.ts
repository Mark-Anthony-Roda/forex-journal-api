...
export default class AuthController {
  public async login({ request, response }: HttpContextContract): Promise<void> {
    ...
    const user = await User.query().where({ email, deletedAt: null }).first()
    ...
    if (remember) jwtCookie = `${jwtCookie} Max-Age=2592000000;`
  ...
  public async authUser({ request, response }: HttpContextContract): Promise<void> {
